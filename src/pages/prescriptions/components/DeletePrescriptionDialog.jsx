...
import { deletePrescription } from "../../../providers/ApiProviders";
import { useQueryClient } from "@tanstack/react-query";
import { toast } from "sonner";

export default function DeletePrescriptionDialog({ deletedPrescriptionRecordInfo }) {
    const queryClient = useQueryClient();
    const { prescription_id, prescribed_by } = deletedPrescriptionRecordInfo;


    const handleDeletePrescription = async () => {
        const promise = async () => {
            try {
                const response = await deletePrescription(prescription_id);
        ...
        <AlertDialog>
            ...
            <AlertDialogContent className="border-t-4 border-t-[#106041] max-w-md">
                ...
                <AlertDialogHeader className="pt-6">
                    ...
                    <div className="mt-2 mb-4 bg-[#f0f8f4] border /30 rounded-md p-3 text-center">
                        <div className="flex items-center justify-center mb-1">
                            ...
                            <span className="font-bold ">Deleting prescription prescribed by {prescribed_by}</span>
                        </div>
                        <div className="text-sm text-gray-600 flex items-center justify-center">
                            <span className="bg-[#106041] text-white text-xs px-2 py-0.5 rounded mr-2">ID</span>
                            ID: {prescription_id || "N/A"}
                        </div>
                    </div>
                    ...
                    <AlertDialogDescription className="text-center">
                        ...
                        <span className="text-gray-600 block">
                            This will permanently delete the prescription record.
                        </span>
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <div className="bg-amber-50 border-l-4 border-amber-400 p-3 my-2 text-sm text-amber-800">
                    ...
                    <p>
                        Deleting this prescription may affect patient assignments and hospital records. Ensure you have proper authorization before proceeding.
                    </p>
                </div>
