...
import { deleteBedType } from "../../../providers/ApiProviders";
import { useBeds } from "../../../providers/ApiContextProvider";
import { toast } from "sonner";

export default function DeleteBedTypesDialog({ children, deletedBedTypeRecordInfo }) {
    const { refreshBedTypes } = useBeds();
    const { id, type_name } = deletedBedTypeRecordInfo;


    const handleDeleteBedType = async () => {
        const promise = async () => {
            try {
                const response = await deleteBedType(id);
                refreshBedTypes();
                return response;
            } catch (err) {
                throw err;
                ...
                <AlertDialogHeader className="pt-6">
                    ...
                    <div className="mt-2 mb-4 bg-[#f0f8f4] border border-[#106041]/30 rounded-md p-3 text-center">
                        <div className="flex items-center justify-center mb-1">
                            ...
                            <span className="font-bold text-[#106041]">{type_name}</span>
                        </div>
