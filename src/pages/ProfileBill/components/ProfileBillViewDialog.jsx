...
import { ProfileEditBillDialog } from "./ProfileBillEditDialog"
                        ...
                        <Card className="border-[#268a6461] pt-0 overflow-hidden">
                            ...
                            <CardContent className="pt-4 space-y-3">
                                <div>
                                    ...
                                    <p className="text-sm font-medium">{bill.surname + " " + bill.first_name || "N/A"}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500">Patient ID</label>
                                    <p className="text-sm font-medium">{bill.patient_id}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500">Hospital Number</label>
                                    <p className="text-sm font-medium">{bill.hospital_number || "N/A"}</p>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border-[#268a6461] pt-0 overflow-hidden">
                            ...
                            <CardContent className="pt-4 space-y-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium">{bill.issued_by}</p>
                                </div>
                                {
                                    bill.updated_by && (
                                        <div>
                                            <label className="text-sm font-medium text-gray-500">Updated By</label>
                                            <p className="text-sm font-medium">{bill.updated_by} at <span className="text-xs text-gray-500">{formatDateTime(bill.updated_at)}</span></p>
                                        </div>
                                    )
                                }
                            </CardContent>
                        ...
                        <CardContent className="pt-4">
                            <div className="grid md:grid-cols-4 gap-4">
                                ...
                                <div className="text-center p-3 bg-[#f0f8f4] rounded-lg border-2 border-[#268a64]">
                                    ...
                                    <p className="text-xl font-bold text-[#106041]">{formatToNaira(bill.total_amount)}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardHeader className="pb-3 pt-3 bg-[#f0f8f4] flex flex-row items-center justify-between">
                            ...
                            </CardTitle>
                            {
                                !(bill.status == "paid" || bill.status == "cancelled") && (
                                    <ProfileEditBillDialog bill={bill}>
                                        <Button
                                            variant="outline"
                                            size="sm"
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-medium capitalize">{bill.payment_method?.replace("_", " ")}</p>
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    <div>
                                        <label className="text-sm font-medium text-gray-500">Amount Paid</label>
                                        <p className="text-sm font-medium">{formatToNaira(bill.amount_paid)}</p>
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium text-gray-500">Payment Date</label>
                                        <p className="text-sm font-medium">{bill.payment_date ? formatDateTime(bill.payment_date) : "N/A"}</p>
                                    </div>
                                </div>
                            </div>
                            {bill.notes && (
                                    ...
                                    <TableBody>
                                        {bill.items?.map((item) => (
                                            <TableRow key={item.id}>
                                                ...
                                                <TableCell>{formatToNaira(item.unit_price)}</TableCell>
                                                <TableCell>{item.quantity}</TableCell>
                                                <TableCell className="text-right font-medium">
                                                    {formatToNaira((Number.parseFloat(item.unit_price) * item.quantity).toString())}
                                                </TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
