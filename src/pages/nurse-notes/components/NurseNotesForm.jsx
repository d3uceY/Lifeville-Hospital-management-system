...
import { createNursesNote } from '../../../providers/ApiProviders'
import { useQueryClient } from '@tanstack/react-query'
import ProfileFormHeader from '../../../components/profile-form-header'

export default function NursesNotesForm() {
    const { user } = useAuth()
    ...
    const schema = z.object({
        ...
        note: z.string().nonempty({ message: "Nurse's note is required" }),
    })
    ...
    const onSubmit = async (data) => {
        ...
        const promise = async () => {
            try {
                const response = await createNursesNote({
                    ...data,
        ...
        toast.promise(promise(), {
            loading: "Saving note...",
            success: `Nurse's note added successfully`,
    ...
    return (
        <div>
            <ProfileFormHeader 
                ...
                description={`Fill in the details to record a nurse's note for patient #${patient_id}`} 
            />
                    ...
                    <CardContent>
                        <div className="mb-4">
                            ...
                            <Textarea
                                ...
                                id="note"
                                placeholder="Write nurse's note here..."
                                {...register("note")}
                            />
                        </div>
