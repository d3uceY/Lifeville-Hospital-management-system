...
import { updateNursesNote } from "../../../providers/ApiProviders"

export default function EditNurseNoteDialog({ children, note }) {
    ...
    const onSubmit = async (data) => {
        const promise = async () => {
            try {
                ...
                setOpen(false)
                await updateNursesNote(note?.id, payload)
                queryClient.invalidateQueries({ queryKey: ["nurseNotes", patient_id] })
                reset()
            } catch (error) {
                console.error(error)
            } finally {
                    ...
                    <Card className="pt-0 mb-8 shadow-sm border-t-4 border-t-[#106041]">
                        <CardHeader className="bg-[#f0f8f4] border-b flex items-center justify-between">
                            <CardTitle className="pt-6 text-xl font-semibold flex items-center gap-2">
                                <FileText size={20} />
                                Edit Nurse Note for {surname} {first_name}
                            </CardTitle>
                            <Button
                                className="mt-6"
                                type="submit"
