...
import { deleteUser } from "../../providers/ApiProviders";
import { toast } from "sonner";
import { useQueryClient } from "@tanstack/react-query";
import { useAuth } from "../../providers/AuthContext";


export default function DeleteUserDialog({ deletedUserRecordInfo }) {
    const queryClient = useQueryClient();
    const { id, name, email } = deletedUserRecordInfo;
    ...
    const handleDeleteUser = async () => {
        const promise = async () => {
            try {
                ...
                // Invalidate users query so it refetches
    ...
    return (
        <AlertDialog>
            <AlertDialogTrigger asChild>
                <Button variant="outline" className="action-delete-btn">
            ...
            <AlertDialogContent className="border-t-4 border-t-[#106041] max-w-md">
                ...
                <AlertDialogHeader className="pt-6">
                    ...
                    <div className="mt-2 mb-4 bg-[#f0f8f4] border /30 rounded-md p-3 text-center">
                        ...
                        <div className="text-sm text-gray-600 flex flex-col items-center justify-center">
                            <span className="bg-[#106041] text-white text-xs px-2 py-0.5 rounded mb-1">
                                ID
                            </span>
                            {id || "N/A"}
                            <span className="text-gray-500 mt-1">{email}</span>
                        </div>
                    </div>
                ...
                <div className="bg-amber-50 border-l-4 border-amber-400 p-3 my-2 text-sm text-amber-800">
                    ...
                    <p>
                        Deleting this user may affect audit logs and system assignments.
                        Ensure you have proper authorization before proceeding.
                    </p>
                </div>
                <AlertDialogFooter className="flex items-center gap-2">
                    <AlertDialogCancel className="hover:bg-[#f0f8f4]">
                        Cancel
                    </AlertDialogCancel>
                    <AlertDialogAction
